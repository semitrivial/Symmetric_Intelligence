...
$\ulcorner s\urcorner$ is never a substring of $\ulcorner s'\urcorner$
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        $p\in (\mathcal O \mathcal R\mathcal A)^*$ such that
...
\begin{exercise} (Permutations)
    \begin{enumerate}
        ...
        for each sequence $s$,
        let $P(s)$ be the result of applying $P$ to all the actions in $s$.
        For each agent $\pi$, let $P(\pi)$ be the agent defined by
        $(P(\pi))(a|s)=\pi(P(a)|P(s))$. For each environment $\mu$, let
        $P(\mu)$ be the environment defined by
        $(P(\mu))(o,r|s)=\mu(o,r|P(s))$. Show that in general
