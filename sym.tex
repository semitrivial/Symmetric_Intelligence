...
\label{omnibusdefn}
    ...
    \begin{enumerate}
        ...
        ORA-prompt $p$ a $\mathbb Q$-valued probability measure on $\mathcal A$.
        For every such $p$ and every $a\in\mathcal A$,
        we write $\pi(a|p)$ for $(\pi(\bullet|p))(a)$.
        Intuitively, $\pi(a|p)$ is the probability that agent $\pi$
        will take action $a$ in response to ORA-prompt $p$.
        \item
        By an \emph{environment}, we mean a function $\mu$
        which assigns to every ORA-play $p$ a $\mathbb Q$-valued probability measure
        (written $\mu(\bullet|p)$)
        on $\mathcal O\times\mathcal R$.
        For every such $p$ and every $(o,r)\in\mathcal O\times\mathcal R$,
        we write $\mu(o,r|p)$ for $(\mu(\bullet|p))(o,r)$.
        Intuitively, $\mu(o,r|p)$ is the probability that environment
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        $U(\mathrm{NEG}\frown x)=\ulcorner \overline m\urcorner$ where
