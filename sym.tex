...
Suppose $U$ is any prefix-free UTM.
\begin{enumerate}
    \item
    For any prefix-free partial computable function $f: 2^*\to 2^*$, the
    ...
    $\forall x\in 2^*$, $f(x)=U(p\frown x)$ (the left-hand side being defined
    if and only if the right-hand side is defined).
    \item
...
\begin{definition}
    For any prefix-free UTM $U$, we define a new prefix-free UTM $U'$ as follows.
    \begin{enumerate}
        \item For every $x\in 2^*$, $U'(0\frown x)=U(x)$.
        \item For every $x\in 2^*$, if $U(x)$ is the code $U(x)=\ulcorner p\urcorner$
        of some $p\in (\mathcal O\times \mathcal R\times\mathcal A)^*$,
        then $U'(1\frown x)=\ulcorner -p\urcorner$. Otherwise, $U(1\frown x)$ is undefined.
    \end{enumerate}
\end{definition}

The following lemma shows that any prefix-free UTM can be modified (as above)
into a prefix-free UTM which assigns the same Kolmogorov complexity to every
environment and its dual environment.

\begin{lemma}
\label{Sissymmetriclemma}
    For each prefix-free UTM $U$, $U'$ really is a prefix-free UTM, with the
    property that for every computable environment $\mu$, $K_{U'}(\mu)=K_{U'}(-\mu)$.
\end{lemma}
...
For every agent $\pi$
and prefix-free UTM $U$, the \emph{Legg-Hutter universal intelligence of $\pi$ given
...
The following theorem shows that for every prefix-free UTM $U$, $\LH_{U'}$
satisfies the above desideratum.

\begin{theorem}
\label{maintheorem}
    For every agent $\pi$ and prefix-free UTM $U$,
    \[
        \LH_{U'}(-\pi) = -\LH_{U'}(\pi).
    \]
\end{theorem}
...
\begin{proof}
    ...
    \begin{align*}
        \LH_{U'}(-\pi) &= \sum_{\mu\in W} 2^{-K_{U'}(\mu)}V^{-\pi}_\mu
            &\mbox{(Definition \ref{universalintelligencedefn})}\\
          &= -\sum_{\mu\in W} 2^{-K_{U'}(\mu)}V^\pi_{-\mu}
...
\label{ignoringrewardscorollary}
    Let $U$ be a prefix-free UTM and
...
\begin{proof}
    ...
    \ref{maintheorem}, $\LH_{U'}(\pi)=-\LH_{U'}(\pi)$,
    forcing $\LH_{U'}(\pi)=0$.
\end{proof}

Corollary \ref{ignoringrewardscorollary} illustrates why it is appropriate, for
purposes of Legg-Hutter universal intelligence, to choose a UTM for which $\mu$
and $-\mu$ have the same Kolmogorov complexity. Consider an agent $\pi_a$
