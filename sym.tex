...
\titlerunning{Symmetric Complexity}
...
would become an environment which always issues reward $-1$
(so is ill-behaved)\footnote{It
is worth mentioning another difference
between these two reward-space-transforms. The hypothetical agent
$\mathrm{AI}_\mu$ with perfect knowledge of the environment's reward distribution
would not change its behavior in response to the transformation $r\mapsto r-1$ (nor indeed
in response to any positive linear scaling of rewards, $r\mapsto ar+b$, $a>0$), but it would
...
``suiciding'' (deliberately terminating the environmental interaction):
one should never walk away from a slot machine that always pays out between $0$ and $1$
dollars, but one should immediately walk away from a slot machine that always pays out
between $-1$ and $0$ dollars.
The agent
AIXI also changes its behavior in response to $r\mapsto r-1$, and it was recently argued that
this can be interpreted in terms of suicide: AIXI models its environment using a mixture
distribution over a countable class of semimeasures, and AIXI's behavior can be interpreted
as treating the complement of the domain of the semimeasure as such a ``suicide'':
