...
The following lemma shows that enviro-symmetric UTMs exist; in fact, any PFUTM
can be modified into one in a simple way.

\begin{lemma}
    Given any PFUTM $U_0$, define a new PFUTM $U$ as follows.
    \begin{enumerate}
        \item For every $x\in 2^*$, $U(0\frown x)=U_0(x)$.
        \item For every $x\in 2^*$, if $U_0(x)=\ulcorner p\urcorner$
        for some $p\in (\mathcal O\times \mathcal R\times\mathcal A)^*$,
