...
is well-known. For example, the range of $\sqcap$ could be composed of 8-bit blocks,
such that every element of the range of $\sqcap$ begins and ends with the ASCII characters
``['' and ``]'', respectively, and such that these characters do not appear anywhere in the
middle.
...
\cite{leike2015bad} ``what are other desirable [UTM properties]?''}.
...
gives reward $0$ while simulating $T$ until $T$ halts, finally giving reward $1$
if $T$ does halt.
