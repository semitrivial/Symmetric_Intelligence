...
\label{doublesubtractionlemma}
    \begin{enumerate}
        ...
        \iten For each environment $\mu$, $-(-\mu)=\mu$.
    \end{enumerate}
\end{lemma}
...
\begin{proof}
    ...
    Case 1: $p$ terminates with a reward.
    Then $p=q\frown \langle o,r\rangle$
    for some ORA-play $q$. We reason:
    \begin{align*}
        X &= \mu(\langle o,r\rangle|q)
            &\mbox{(Definition of $X$)}\\
          &= \mu(\langle o,--r\rangle | {--q})
    ...
    Case 2: $p$ terminates with an action.
    Then $p=q\frown a$ for some ORA-prompt $q$. We reason:
    \begin{align*}
        X &= \pi(a|q)
            &\mbox{(Definition of $X$)}\\
          &= \pi(a|{--q})
